...
def prepare_commit_transaction(app_id: int, program_id: int, program_account: str, pool_asset_id: int, amount: int, sender: str, suggested_params, nft_asset_id: Optional[int] = None):
    ...
    if nft_asset_id is not None:
        assert nft_asset_id > 0, "Invalid nft asset id"
        nft_log_balance_txn = ApplicationNoOpTxn(
            index=app_id,
            sender=sender,
            sp=suggested_params,
            app_args=['log_balance', ],
            foreign_assets=[nft_asset_id],
        )
